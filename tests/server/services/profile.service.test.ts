...
describe('ProfileService', () => {
  ...
  it('does not touch display_name when displayName is omitted', async () => {
    await service.completeOnboarding({ email: 'a@example.com', dateOfBirth: '1995-06-15', gender: 'male', height: 178, weight: 75 })
  ...
  it('writes an explicitly empty displayName instead of silently ignoring it', async () => {
    await service.completeOnboarding({ email: 'a@example.com', dateOfBirth: '1995-06-15', gender: 'male', height: 178, weight: 75, displayName: '' })
  ...
  it('includes displayName from users when reading the profile back', async () => {
    await service.completeOnboarding({ email: 'a@example.com', displayName: 'Jordan', dateOfBirth: '1995-06-15', gender: 'male', height: 178, weight: 75 })
  ...
  it('converts weight using the profile\'s actual resolved unitSystem, not a stale pre-fetched guess', async () => {
    await service.completeOnboarding({ email: 'a@example.com', dateOfBirth: '1995-06-15', gender: 'male', height: 70, weight: 180, unitSystem: 'imperial' })

    // Second call omits unitSystem — weight must still be interpreted as lbs (preserved imperial),
    // not misread as already-kg.
    await service.completeOnboarding({ email: 'a@example.com', dateOfBirth: '1995-06-15', gender: 'male', height: 71, weight: 184 })
  ...
  it('creates an active weight target when targetWeight is provided', async () => {
    await service.completeOnboarding({ email: 'a@example.com', dateOfBirth: '1995-06-15', gender: 'male', height: 178, weight: 90, targetWeight: 80 })
  ...
  it('converts targetWeight using the resolved unitSystem, same as weight', async () => {
    await service.completeOnboarding({ email: 'a@example.com', dateOfBirth: '1995-06-15', gender: 'male', height: 70, weight: 180, targetWeight: 160, unitSystem: 'imperial' })
  ...
  it('does not create a duplicate active weight target on a repeat onboarding call', async () => {
    await service.completeOnboarding({ email: 'a@example.com', dateOfBirth: '1995-06-15', gender: 'male', height: 178, weight: 90, targetWeight: 80 })
    await service.completeOnboarding({ email: 'a@example.com', dateOfBirth: '1995-06-15', gender: 'male', height: 178, weight: 88, targetWeight: 78 })
  ...
  it('creates the users row itself when the caller has no prior row (no signup flow exists yet)', async () => {
    ...
    await freshService.completeOnboarding({ email: 'brand-new@example.com', displayName: 'Brand New', dateOfBirth: '1995-06-15', gender: 'male', height: 178, weight: 75 })
